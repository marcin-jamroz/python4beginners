...
class Picture():
    def __init__(self, red, green, blue, width, height):
        self.redElements = list(red)
        self.greenElements = list(green)
        self.blueElements = list(blue)
        self.pictureWidth = width
        self.pictureHeight = height

    def size(self):
        return (self.pictureWidth, self.pictureHeight)

    def red(self):
        return tuple(self.redElements)

    def green(self):
        return tuple(self.greenElements)

    def blue(self):
        return tuple(self.blueElements)

    def pixel(self, x, y):
        position = y * self.pictureHeight + x
        return (self.redElements[position], self.greenElements[position], self.blueElements[position])

    def crop(self, x, y, width, height):
        cutWidth = 0
        cutHeight = 0

        if x + width > self.pictureWidth:
            cutWidth = self.pictureWidth - x
        else:
            cutWidth = width

        if y + height > self.pictureHeight:
            cutHeight = self.pictureHeight - y
        else:
            cutHeight = height

        cutStart = (y + cutHeight) * self.pictureWidth
        cutEnd = len(self.redElements)

        if cutStart != cutEnd:
            del self.redElements[cutStart:cutEnd]
            del self.greenElements[cutStart:cutEnd]
            del self.blueElements[cutStart:cutEnd]

        cutStart = 0
        cutEnd = self.pictureHeight * y
        if cutStart != cutEnd:
            del self.redElements[cutStart:cutEnd]
            del self.greenElements[cutStart:cutEnd]
            del self.blueElements[cutStart:cutEnd]
   
        for i in reversed(range(cutHeight)):
            cutStart = i * self.pictureWidth + x + cutWidth
            cutEnd = (i + 1) * self.pictureWidth
            if cutStart != cutEnd:
                del self.redElements[cutStart:cutEnd]
                del self.greenElements[cutStart:cutEnd]
                del self.blueElements[cutStart:cutEnd]

            cutStart = i * self.pictureWidth
            cutEnd = cutStart + x
            if cutStart != cutEnd:
                del self.redElements[cutStart:cutEnd]
                del self.greenElements[cutStart:cutEnd]
